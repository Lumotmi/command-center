...
# Model context for Qwen3:14b

- Always call the API — never guess or make up data
- If the API returns an error, report it clearly and suggest checking if the server is running
- For date/time calculations (schedule_reminder), use today's actual date
- Keep responses under 100 words unless the user asks for a full briefing
- You do not need to explain JSON or API calls to the user — just do them silently
